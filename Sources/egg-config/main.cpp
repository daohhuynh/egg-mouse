...
// The four LOD options the capture exercised were 0..10 inclusive, eleven of
// them, one APPLY each (windows-run/02-basic.pcapng lines 19-29). The dropdown
// has eleven entries and the twelfth slot in the log found nothing to select.
...
// Put a field value into its byte without touching anything else in it.
...
int cmdEncode(const std::string& field, const std::string& value,
    ...
    rec[at] = composeByte(before[at], f, want);
...
int cmdSet(const std::string& field, const std::string& value,
    ...
    if (!yes) {
        if (f->mask == 0xFF) {
            ...
            std::printf("set %s = %ld would write field value 0x%02x into the"
                        ...
                        f->name, v, want, f->mask, f->recordOffset,
                        kPayloadOffset + f->recordOffset);
        }
    ...
    warnIfActiveStageWouldBeOutOfRange(before, f, v);
    const std::size_t at = kPayloadOffset + f->recordOffset;
    want = composeByte(before[at], f, want);
    if (before[at] == want) {
